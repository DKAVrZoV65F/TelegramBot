...
CREATE TABLE messages (
    id          BIGINT NOT NULL,        -- Telegram message_id (уникален в чате)
    ...
    CONSTRAINT pk_messages PRIMARY KEY (id, chat_id)  -- (id) не уникален глобально
);
...
CREATE TABLE notes (
    ...
    chat_id     BIGINT,
    author      VARCHAR(64),
    note_text   BLOB SUB_TYPE TEXT,
    created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_notes_msg FOREIGN KEY (message_id, chat_id)
        REFERENCES messages(id, chat_id)
        ON DELETE SET NULL
);
