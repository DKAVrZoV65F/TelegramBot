...
from handlers.statistics import cmd_statistics, process_back_to_type_choice, process_statics_period, \
    process_statistics_request, process_statistics_type_choice
from keyboards import admin_menu, period_menu
from utils.util import make_msg_link
from db import get_conn
from datetime import datetime, timedelta
import io, xml.etree.ElementTree as ElementTree
...
async def send_unprocessed_xml(call: types.CallbackQuery):
  ...
  """
    ...
    for msg_id, chat_id, dt, user_id, username, text, raw_json, tags, sentiment, processed in rows:
        m = ElementTree.SubElement(root, "message", {
            "id": str(msg_id),
            "chat_id": str(chat_id),
        ...
        tags_el = ElementTree.SubElement(m, "tags")
        for tag in tags.split(","):
            if tag:
                ElementTree.SubElement(tags_el, "tag", {"name": tag})

        text_el = ElementTree.SubElement(m, "text")
        text_el.text = f"<![CDATA[{text}]]>"

        if raw_json:
            raw_el = ElementTree.SubElement(m, "raw_json")
            raw_el.text = f"<![CDATA[{raw_json}]]>"

    buf = io.BytesIO()
    ElementTree.ElementTree(root).write(buf, encoding="utf-8", xml_declaration=True)
    buf.seek(0)
...
def register(dp):
    ...
    dp.register_callback_query_handler(process_statics_period, lambda c: c.data.startswith("stats_fetch:"))
