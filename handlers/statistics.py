...
def statistics_period_buttons() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(row_width=2).add(
        InlineKeyboardButton("7 дней", callback_data="stats_fetch:7"),
        InlineKeyboardButton("30 дней", callback_data="stats_fetch:30")
    )
    ...
    try:
        ...
        if positive_data:
            ...
            if positive_chart_buffer:
                ...
                await call.message.answer(f"Не удалось сгенерировать график для позитивной статистики (данные есть, но график пуст).")
        else:
        ...
        if negative_data:
            ...
            if negative_chart_buffer:
                ...
                await call.message.answer(f"Не удалось сгенерировать график для негативной статистики (данные есть, но график пуст).")
...
async def fetch_tag_trends_data(days: int) -> dict:
    """
    Извлекает данные для построения трендов по тегам (количество по дням).
    Возвращает словарь: {'тег1': {дата1: кол-во, дата2: кол-во}, 'тег2': ...}
    """
    since_date = datetime.utcnow() - timedelta(days=days)
    conn = get_conn()
    cur = conn.cursor()
...
async def process_statistics_period(call: types.CallbackQuery):
    ...
    try:
        ...
        if positive_data_pie:
            ...
            await call.message.answer(f"Нет данных для пироговой диаграммы позитивной статистики за {period_days} дней.")
        ...
        if negative_data_pie:
            ...
            await call.message.answer(f"Нет данных для пироговой диаграммы негативной статистики за {period_days} дней.")
        ...
        if tag_trends_data:
            ...
            if trends_chart_buffer:
                ...
                await call.message.answer(f"Не удалось сгенерировать график трендов (возможно, нет данных для отображения).")
        else:
...
async def fetch_top_n_tags(days: int, top_n: int = 5) -> dict:
    """
    Извлекает топ-N тегов по количеству упоминаний, раздельно для positive/negative.
    Возвращает: {'positive': [('тег', кол-во), ...], 'negative': [('тег', кол-во), ...]}
    """
    since_date = datetime.utcnow() - timedelta(days=days)
    conn = get_conn()
    cur = conn.cursor()
        ...
        if report_type == "pie":
            ...
            if tag_trends_data:
                ...
                if trends_chart_buffer:
                    ...
                    await call.message.edit_text(f"Не удалось сгенерировать график трендов за {period_days} дней (возможно, нет данных для отображения).")
            else:
