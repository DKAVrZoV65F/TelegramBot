...
async def send_weekly_statistics_report(bot: Bot, admin_id: int, period_days: int = 7):
    ...
    try:
        ...
        if top_n_tags_data.get("positive"):
            ...
            top_tags_text_parts.append(f"👍 Топ-{len(top_n_tags_data['positive'])} позитивных тегов:\n{positive_top_list}")
        
        if top_n_tags_data.get("negative"):
            negative_top_list = "\n".join([f"  - {tag} ({count})" for tag, count in top_n_tags_data["negative"]])
            top_tags_text_parts.append(f"👎 Топ-{len(top_n_tags_data['negative'])} негативных тегов:\n{negative_top_list}")
        ...
        try:
            ...
            logger.error(f"Не удалось отправить сообщение об ошибке еженедельного отчета admin_id {admin_id}: {e_send_err}")
