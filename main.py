...
def setup_schedulers(bot_instance: Bot):
    try:
        ...
        logger.warning(f"Не удалось загрузить timezone {settings.TIMEZONE_STR} из zoneinfo, пробуем pytz или UTC.")
        try:
            tz = timezone(settings.TIMEZONE_STR)
        except Exception:
            logger.error(f"Не удалось установить timezone {settings.TIMEZONE_STR}. Используется UTC.")
            tz = None
    ...
    if hasattr(settings, 'DAILY_REPORT_HOUR') and hasattr(settings, 'DAILY_REPORT_MINUTE'):
        ...
        logger.info(
            f"Ежедневный отчет запланирован на {settings.DAILY_REPORT_HOUR}:{settings.DAILY_REPORT_MINUTE} ({settings.TIMEZONE_STR}).")
    ...
    if hasattr(settings, 'WEEKLY_REPORT_HOUR') and hasattr(settings, 'WEEKLY_REPORT_MINUTE'):
        ...
        logger.info(
            f"Еженедельный статистический отчет запланирован на пятницу {settings.WEEKLY_REPORT_HOUR}:{settings.WEEKLY_REPORT_MINUTE} ({settings.TIMEZONE_STR}).")
    else:
        logger.warning(
            "Время для еженедельного отчета не настроено в settings (WEEKLY_REPORT_HOUR, WEEKLY_REPORT_MINUTE).")
